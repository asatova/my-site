...
function OnEarlyContentComplete( $buffer, $tmpUpdate = false )
{
	...
	{
		if( !headers_sent() )
			header( 'X-Seraph-Accel-Content: 2.20.46; status=' . $skipStatus . ', procTime=' . $tmProc . 's' );
	...
	if( $ctxProcess[ 'mode' ] == 'full' )
	{
		...
		if( $aFreshItemClassApply )
		{
...

			{
				...
				if( apply_filters( 'seraph_accel_jscss_addtype', false ) )
					$item -> setAttribute( 'type', 'text/css' );
				$item -> nodeValue = htmlspecialchars( '[data-lzl-fr].lzl-fr-ing:not(.lzl-fr-sa) *{opacity:0;visibility:hidden;}' . ( Gen::GetArrField( $settContPr, array( 'fresh', 'smoothAppear' ), false ) ? '[data-lzl-fr]:not(.lzl-fr-ing) *{transition:opacity .25s ease-in-out;}' : '' ) );
